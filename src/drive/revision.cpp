...
{
    if (!map.contains(QLatin1StringView("kind")) || map[QStringLiteral("kind")].toString() != QLatin1String("drive#revision")) {
        return RevisionPtr();
    }
...
RevisionsList Revision::fromJSONFeed(const QByteArray &jsonData)
{
    ...
    if (!map.contains(QLatin1StringView("kind")) || map[QStringLiteral("kind")].toString() != QLatin1String("drive#revisionList")) {
        return RevisionsList();
    }
